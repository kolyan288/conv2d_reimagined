...
class LatencyMetricsWriter:
    ...
    def record_metrics(
        ...
        latency_cpu_ms = self.extract_cpu_latency_from_profiler(cpu_profiler) if cpu_profiler else None
        
        # Extract GPU latency
        latency_gpu_mean_ms, latency_gpu_std_ms = gpu_latency if gpu_latency else (None, None)
